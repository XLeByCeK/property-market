...
async function searchProperties(aiFilters) {
    ...
    if (aiFilters.transactionType) {
        const tType = await prisma_1.default.transaction_type.findFirst({
    ...
    if (aiFilters.propertyType) {
        const pType = await prisma_1.default.property_type.findFirst({
    ...
    if (aiFilters.rooms) {
        if (Array.isArray(aiFilters.rooms)) {
            where.rooms = { in: aiFilters.rooms.map((r) => Number(r)) };
        }
        else {
