...
export async function parseUserQuery(message: string) {
  try {
    ...
    - findBestDeal: boolean. true — если пользователь явно или косвенно просит найти самый ВЫГОДНЫЙ
      ...
      "помоги выбрать"). По умолчанию true — ассистент всегда отмечает один лучший вариант.

    Если параметр не указан в запросе, ставь null (кроме findBestDeal — он по умолчанию true).
  `
        },
