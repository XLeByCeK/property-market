...
interface PropertyContactCardProps {
  // any, потому что favorites-флаг приходит из разных мест и не входит в строгий PropertyDetails.
  property: any;
}

const SelfMessageModal: React.FC<{ onClose: () => void }> = ({ onClose }) => (
    ...
    <>
      <div className="property-contact-card">
        ...
        <div className="contact-card-buttons">
          <button className="contact-card-button phone-button" onClick={handleShowPhone}>
            ...
            <span>{phoneVisible ? property.user.phone : 'Показать телефон'}</span>
          </button>
          ...
          >
            ...
            <span>{isSending ? 'Переход...' : 'Написать'}</span>
          </button>
        </div>
      </div>
      ...
      {isSelfMessageModalOpen && (
        <SelfMessageModal onClose={() => setIsSelfMessageModalOpen(false)} />
      )}
    </>
  );
};
